...
nagents=100
ntries=500
strategy=5	# Strategy of the agents dynamic cases
...
fOut=open("Dynamic1000-f"strftime("%Y-%m-%d %H:%S", ftime)".dat","w+")
fOutCsv=open("Dynamic1000-f"strftime("%Y-%m-%d %H:%S", ftime)".csv","w+")
fOutCsvD=open("DynamicD1000-f"strftime("%Y-%m-%d %H:%S", ftime)".csv","w+")
