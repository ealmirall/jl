# Final for revision 2 using AWS
#    jump bt 2.. bits
#    No classes
#    Count iterations
...
_dpivot=0	#probability to jump proportional to the distance to bench
...
nBestCases=10
maxPivots=100000	#this is a mean
_niter=0			#we count the number of iterations


nagents=100
ntries=500
#strategy=3	# Strategy of the agents 3 for using avg of explorers
strategy=2 #Strategy of the agents 2 - using the max of explorers
...
#fOut=open("NK1stgAvg"strftime("%Y-%m-%d %H:%S", ftime)".dat","w+")
#fOutCsv=open("NK1stgAvg"strftime("%Y-%m-%d %H:%S", ftime)".csv","w+")
#fOutCsvD=open("NK1stgAvgD"strftime("%Y-%m-%d %H:%S", ftime)".csv","w+")

fOut=open("MaxP-UCNP"strftime("%Y-%m-%d %H:%S", ftime)".dat","w+")
fOutCsv=open("MaxP-UCNP"strftime("%Y-%m-%d %H:%S", ftime)".csv","w+")
fOutCsvD=open("MaxPD-UCNP"strftime("%Y-%m-%d %H:%S", ftime)".csv","w+")
